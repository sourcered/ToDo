...
const std::string SQL_CREATE_TABLE        = "CREATE TABLE IF NOT EXISTS todo (position INT NOT NULL UNIQUE PRIMARY KEY AUTO_INCREMENT, task VARCHAR(60) NOT NULL DEFAULT \'No task...\')";
const std::string SQL_ADD_TASK            = "INSERT INTO todo (task,position) VALUES (?,?)";
...
const std::string SQL_GET_TASK            = "SELECT task FROM todo WHERE position=?";
const std::string SQL_GET_LAST_POSITION   = "SELECT position FROM todo ORDER BY position DESC LIMIT 1";
const std::string SQL_DROP_POSITION       = "ALTER TABLE todo DROP position";
const std::string SQL_AUTOINCREMENT_1     = "ALTER TABLE todo AUTO_INCREMENT = 1";
const std::string SQL_ADD_POSITION_COLUMN = "ALTER TABLE todo ADD position INT NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST";
