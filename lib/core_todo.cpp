#include "../include/core_todo.h"

namespace ToDo
{

    sql::Connection * Factory::getConnection()
    {
        sql::Driver * driver;
        sql::Connection * con;

        driver = get_driver_instance();
        con = driver->connect("tcp://127.0.0.1:3306", "root", "1234");
        con->setSchema("DB_ToDo");

        return con;
    }

    bool Factory::closeConnection(sql::Connection * con)
    {
      try
      {
          con->close();
          delete con;
          return true;
      }
      catch(sql::SQLException & ex) { return false; }
    }

    bool Factory::closePreparedStatement(sql::PreparedStatement * pstm)
    {
        try
        {
            pstm->close();
            delete pstm;
            return true;
        }
        catch(sql::SQLException & ex) { return false; }
    }

    bool Factory::init(sql::Connection * con)
    {
        try
        {
            sql::Statement * stm;
            stm = con->createStatement();
            stm->execute(SQL_CREATE_TABLE);
            stm->close();
            delete stm;
            return true;
        }
        catch(sql::SQLException & ex) { return false; }
    }

    bool Factory::SQL_add_task_order(std::string task, int order)
    {
        try
        {
            sql::Connection * con;
            con = getConnection();
            sql::PreparedStatement * pstm;
            pstm = con->prepareStatement(SQL_ADD_TASK_ORDER);
            pstm->setString(1,task);  //Task
            pstm->setInt(2, order);     //Order
            if(closeConnection(con)) delete con;
            if(closePreparedStatement(pstm)) delete pstm;
            return true;
        }
        catch(sql::SQLException & ex) { return false; }
    }

    bool Factory::SQL_add_task(std::string task)
    {
        try
        {
            sql::Connection * con;
            con = getConnection();

            sql::PreparedStatement * pstm;
            pstm = con->prepareStatement(SQL_ADD_TASK);
            pstm->setString(1,task);  //Task

            // ----------GET LAST ORDER------------------
            int order = 1;  //Get order
            pstm->setInt(2,order);  //Add to the last element Order
            // ------------------------------------------
            pstm->executeUpdate();

            if(closeConnection(con)) delete con;
            if(closePreparedStatement(pstm)) delete pstm;

            return true;
        }
        catch(sql::SQLException & ex) { return false; }
    }

    bool Factory::SQL_remove_task(int order)
    {
        try
        {
            return true;
        }
        catch(sql::SQLException & ex) { return false; }
    }

    bool Factory::SQL_update_task(std::string task, int order)
    {
        try
        {
            return true;
        }
        catch(sql::SQLException & ex) { return false; }
    }

    bool Factory::SQL_update_order(int order)
    {
        try
        {
            return true;
        }
        catch(sql::SQLException & ex) { return false; }
    }

    bool Factory::SQL_update_task_order(std::string task, int order, int newOrder)
    {
        try
        {
            return true;
        }
        catch(sql::SQLException & ex) { return false; }
    }

    bool Task::add(std::string)
    {
        return false;
    }

    bool Task::remove(int )
    {
        return false;
    }

    std::vector<std::string> View::getTasks()
    {
        std::vector<std::string> tmp;
        return tmp;
    }

    std::vector<std::string> View::getTasks(order ord)
    {
        std::vector<std::string> tmp;
        switch (ord)
        {
            case asc:
                break;
            case desc:
                break;
            default:
                std::cout << "Erro get Order..." << std::endl;
                break;
        }

        return tmp;
    }

    std::vector<int> View::getPostions()
    {
        std::vector<int> tmp;
        return tmp;
    }

    std::string View::getTasksByPostion(int)
    {
        std::string tmp;
        return tmp;
    }

}
